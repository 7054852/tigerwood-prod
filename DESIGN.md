...
### Tertiary: цвета направлений
Цвет значка показывает, **где я нахожусь**, — только значки меню, шапка раздела и её плитка. Не для кнопок, статусов и заливок.
- **Продажи — фиолетовый** (#6E56CF на #EFEBFB; ночью #A897F3 на #27213F).
- **Каталог и расценки — бирюзовый** (#0F8A7E на #E1F3F0; ночью #4CC4B6 на #12302C).
- **Производство — тёплое дерево** (#C8660F на #FBEBDA; ночью #F2A35A на #36250F).
- **Снабжение — оливковый** (#4F8A2E на #E8F2E1; ночью #94C96F на #1D2B15).
- **Кадры — розовый** (#D0457A на #FBE7EF; ночью #F28DB1 на #3B1B28).
- **Администрирование — графит** (#56647C на #E8ECF2; ночью #A7B1C4 на #242935).
Применяется атрибутом `data-g="sales|catalog|prod|supply|hr|admin"` → переменные `--dc`/`--dc-bg`; плитка-значок `.dtile`.
...
**Каркас раздела — одинаковый порядок на каждом экране:** шапка оболочки (цветная плитка направления + направление + название) → вкладки `.tabs` (всегда первые, без эмодзи) → панель `.toolbar` (слева поиск, фильтры, листалка `.pnav`, счётчик `.meta`; справа `.end`: «Обновить», вторичные, основная синяя — последней) → чипы `.chips` → уведомления `.note` → плитки `.tiles` → группы `.sect` с карточками и таблицами. Режимы (день/ночь смены, период, «ставлю график / отмечаю выход») — сегментный переключатель `.seg`.
